...
- **URL**: https://func-irail-staging-{random-suffix}.azurewebsites.net
...
> **Note**: The `{random-suffix}` is automatically generated by Terraform to ensure global uniqueness across all Azure subscriptions. The naming follows the pattern `func-{project}-{environment}-{random-suffix}` for Function Apps and `rg-{project}-{environment}-{random-suffix}` for Resource Groups.
...
- **Environment Separation**: Clear distinction between staging and production environments
...
```
# For random suffix "a1b2c3d4"
Function App: func-irail-staging-a1b2c3d4
Resource Group: rg-irail-staging-a1b2c3d4  
URL: https://func-irail-staging-a1b2c3d4.azurewebsites.net

# Alternative environments:
Production: func-irail-production-a1b2c3d4
Development: func-irail-dev-a1b2c3d4
```
...
### 2. Environment Configuration Setup (New Approach)

The project now uses dedicated `.env` files for each environment instead of a single incomplete `.env.template`. This provides better security and environment separation.
...
└── .env.template         # Legacy template (incomplete - do not use)
```

#### Setting Up Staging Environment
The `.env.staging` file is already configured with all necessary variables:

```bash
# Staging Environment Variables for Terraform
export TF_VAR_sql_admin_username=sqladmin
export TF_VAR_sql_admin_password=MiLolita421+
export TF_VAR_developer_ip=  # Leave empty to skip developer IP access
export TF_VAR_environment=staging
export TF_VAR_location="West Europe"
export TF_VAR_sql_sku=Basic
export TF_VAR_app_service_plan_sku=FC1
```
...
export TF_VAR_sql_admin_username=sqladmin
export TF_VAR_sql_admin_password=YourProductionPassword123!
export TF_VAR_developer_ip=  # Leave empty for security
export TF_VAR_environment=production
export TF_VAR_location="West Europe"
export TF_VAR_sql_sku=Standard
export TF_VAR_app_service_plan_sku=EP1
```
...
- ✅ **DO**: Keep sensitive values in separate files per environment
- ✅ **DO**: Add `.env.*` files to `.gitignore` if they contain real passwords
- ❌ **DON'T**: Use the legacy `.env.template` (incomplete)
- ❌ **DON'T**: Commit production passwords to git
- ❌ **DON'T**: Use the same password for staging and production

#### Why This Approach?
1. **Complete Configuration**: No missing variables like the old template
2. **Environment Separation**: Clear distinction between staging and production
3. **Automated Integration**: Works seamlessly with deployment scripts
4. **Security**: Environment-specific credentials and settings
5. **CI/CD Ready**: Variables map directly to Azure DevOps variable groups

### 3. Configure Service Connections (Critical Step)
...
### 4. Configure Variable Groups (Essential Configuration)
...
```
AZURE_FUNCTION_APP_NAME: func-irail-staging-{random-suffix}
